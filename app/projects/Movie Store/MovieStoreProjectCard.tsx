...
import MovieStoreModal from "./MovieStoreModal";

export default function MovieStoreProjectCard() {
    ...
    return (
        ...
        <div onClick={() => setOpen(true)} className="hover:shadow-md transiton-shadow hover:bg-gray-100 rounded-2xl bg-white backdrop-blur-md border border-white/70 shadow-lg p-6 ring-1 ring-black/10">
            <h3 className = "text-slate-900 mb-2 text-lg font-bold font-montserrat">Movie Store</h3>
            <Image
                src={"/MovieStore/HomePage.png"}
                alt={"HomePage"}
                width={1000}
                height={1000}
            />
        ...
        {open && <MovieStoreModal onClose={() => setOpen(false)} />}
        </>
    )
}